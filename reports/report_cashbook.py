    ...
    def _get_account_move_entry(self, accounts, init_balance, sortby, display_account):
        ...
        if init_balance:
            ...
            sql = ("""
                    ...
                    WHERE m.create_uid IN %s AND l.account_id IN %s""" + filters + 'GROUP BY l.account_id, l.pos_reference', 'l.room_ref')
        ...
        sql = ('''SELECT l.id AS lid, l.account_id AS account_id, l.date_with_time AS ldate, j.code AS lcode, l.currency_id, l.amount_currency, l.ref AS lref, l.name AS lname, l.pos_reference AS pos_reference, l.room_ref AS room_ref, COALESCE(l.debit,0) AS debit, COALESCE(l.credit,0) AS credit, COALESCE(SUM(l.debit),0) - COALESCE(SUM(l.credit), 0) AS balance,\
                        ...
                        JOIN account_journal j ON (l.journal_id=j.id)\
                        JOIN account_account acc ON (l.account_id = acc.id) \
                        WHERE m.create_uid IN %s AND l.account_id IN %s ''' + filters + ''' GROUP BY l.id, l.account_id, l.date_with_time, j.code, l.currency_id, l.amount_currency, l.ref, l.name, l.pos_reference, 'l.room_ref', m.name, c.symbol, p.name ORDER BY ''' + sql_sort)
